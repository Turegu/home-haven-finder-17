...
const EventGridCard = memo(({ event }: EventGridCardProps) => {
    ...
    <Link to={`/events/${event.id}`}>
      <div className="group bg-card rounded-xl border border-border overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
        <div className="relative aspect-[4/3] overflow-hidden bg-muted">
          ...
          {images.length > 1 && (
            <>
              <button onClick={prevImage} className="absolute left-2 top-1/2 -translate-y-1/2 bg-foreground/40 hover:bg-foreground/60 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity"><ChevronLeft className="h-4 w-4" /></button>
              <button onClick={nextImage} className="absolute right-2 top-1/2 -translate-y-1/2 bg-foreground/40 hover:bg-foreground/60 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity"><ChevronRight className="h-4 w-4" /></button>
            </>
          )}
          <div className="absolute bottom-2 left-2 flex items-center gap-1 bg-foreground/60 text-white text-xs px-2 py-1 rounded-md">
            <Camera className="h-3 w-3" /><span>{currentImage + 1}/{images.length}</span>
          </div>
          <div className="absolute top-2 left-2 flex flex-col gap-1.5 items-start">
            <TierBadge displayOnHomepage={event.display_on_homepage} />
            {event.entry_type === 'open_invitation' && (
              <Badge className="bg-green-600 hover:bg-green-600 text-white border-0 text-[10px] uppercase font-bold gap-1"><Users className="h-3 w-3" /> Open Invitation</Badge>
            )}
          </div>
        ...
        <div className="p-4">
          <div className="flex items-center justify-between mb-1">
            ...
            {companyLogo && <img src={companyLogo} alt="" className="h-7 w-auto max-w-[60px] rounded-lg object-contain" />}
          </div>
          ...
          <div className="flex items-center gap-3 pt-3 border-t border-foreground/10">
            <div className="flex items-center gap-1 text-muted-foreground text-xs"><CalendarDays className="h-3.5 w-3.5" /><span>{formatDate(event.event_date)}</span></div>
            <div className="flex items-center gap-1 text-muted-foreground text-xs"><TypeIcon className="h-3.5 w-3.5" /><span>{event.event_type.replace(/_/g, ' ')}</span></div>
          </div>
        </div>
      </div>
    </Link>
