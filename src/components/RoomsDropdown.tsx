...
export default function RoomsDropdown({ value, onChange, label }: RoomsDropdownProps) {
  const { t } = useTranslation();
  const { options: fo } = useFilterOptions("search");
  const roomOptions = fo["rooms"] || [];
  const [search, setSearch] = useState("");

  const displayLabel = label || t('searchFilters.rooms');
      ...
      <PopoverTrigger asChild>
        <button className="flex items-center gap-1.5 px-3 py-2 text-sm border border-border rounded-md hover:border-primary/50 transition-colors bg-background min-w-[80px]">
          <span className={value.length > 0 ? 'text-foreground' : 'text-muted-foreground'}>
            {value.length > 0 ? value.join(', ') : displayLabel}
          </span>
          {value.length > 0 && (
        ...
        <div className="p-2 border-b border-border">
          <div className="relative">
            ...
            <input
              ...
              placeholder={t('searchFilters.searchRooms')}
              className="w-full h-7 ps-7 pe-2 rounded border border-input bg-background text-sm focus:outline-none focus:ring-1 focus:ring-ring placeholder:text-muted-foreground"
            />
          </div>
        </div>
        <div
