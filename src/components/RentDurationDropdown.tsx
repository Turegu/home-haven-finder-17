...
export default function RentDurationDropdown({ value, onChange }: RentDurationDropdownProps) {
  ...
  const visible = expanded ? durationOptions : durationOptions.slice(0, VISIBLE_COUNT);
  const hasMore = durationOptions.length > VISIBLE_COUNT;
        ...
        {hasMore && (
          ...
          >
            {expanded ? (
              ...
              <>Show More ({durationOptions.length - VISIBLE_COUNT}) <ChevronDown className="h-3 w-3" /></>
            )}
          </button>
        )}
      </PopoverContent>
