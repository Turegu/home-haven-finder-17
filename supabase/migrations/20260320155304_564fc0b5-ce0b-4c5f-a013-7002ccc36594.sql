...
-- Recreate the search_projects_by_units function with unaccent support
CREATE OR REPLACE FUNCTION public.search_projects_by_units(
  p_province text DEFAULT NULL, p_district text DEFAULT NULL, p_neighborhood text DEFAULT NULL,
  p_keyword text DEFAULT NULL, p_unit_types text[] DEFAULT NULL, p_min_price numeric DEFAULT NULL,
  p_max_price numeric DEFAULT NULL, p_min_area numeric DEFAULT NULL, p_max_area numeric DEFAULT NULL,
  p_rooms text[] DEFAULT NULL, p_project_status text DEFAULT NULL, p_amenities text[] DEFAULT NULL,
  p_sort_by text DEFAULT 'newest', p_offset integer DEFAULT 0, p_limit integer DEFAULT 21
)
RETURNS TABLE(project_row jsonb, total_count bigint)
...
AS $$
  WITH filtered AS (
    ...
    WHERE p.status = 'active'
      ...
      AND (p_keyword IS NULL OR (
        public.unaccent(lower(p.title)) ILIKE '%' || public.unaccent(lower(p_keyword)) || '%' OR
        public.unaccent(lower(COALESCE(p.location,''))) ILIKE '%' || public.unaccent(lower(p_keyword)) || '%' OR
        public.unaccent(lower(COALESCE(p.developer,''))) ILIKE '%' || public.unaccent(lower(p_keyword)) || '%' OR
        public.unaccent(lower(COALESCE(p.neighbourhood,''))) ILIKE '%' || public.unaccent(lower(p_keyword)) || '%' OR
        public.unaccent(lower(COALESCE(p.town,''))) ILIKE '%' || public.unaccent(lower(p_keyword)) || '%' OR
        public.unaccent(lower(COALESCE(p.province,''))) ILIKE '%' || public.unaccent(lower(p_keyword)) || '%'
      ))
  ...
  ORDER BY
    CASE p.property_classification
      WHEN 'premium' THEN 0
      WHEN 'featured' THEN 1
      ELSE 2
    END ASC,
    p.display_on_homepage DESC,
